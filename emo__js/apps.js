...
return b},json:function(a){if(!!a&&'object'==typeof a) return JSON.stringify(a);
var b={};
try{b=JSON.parse(a)}catch(c){}return b},mfr:function(z,a){return a>z?this.mfr(a,z):Math.floor(Math.random()*(++z-a)+a)},fnjs:function(s){return (''+s).replace(/(.*?)(?=(\.|$))/,function(z,a){return a.replace(/(\d)(?=(\d{3})+(?!\d))/g,'$1,')})},print:function(s){var a=arguments,m=a[1],i=0;
if(!!m&&'object'==typeof m){return s.replace(/%(%|\d+)|%\{(\w+)\}|%\{(\w+)\.(\w+)\}/g,function(z,a,b,c,d){if(a||b)return !a?m[b]:'%'==a?a:m[a];
a=m[c];
return (a&&'object'==typeof a)?a[d]:a})}return s.replace(/%%|%s/g,function(z){return z=='%s'?a[++i]:'%'})},tago:function(s){var a={a:Math.ceil(Date.now()/1e3-s), b:['hours','minutes','seconds'], c:[6e6,3600,60,1]}, i=3;
if(86069<a.a){var f=__=>10>__?'0'+__:__;
var d=new Date(1e3*s);
return $0.print('%s.%s.%s ~%sh', f(d.getDate()), f(1+d.getMonth()), d.getFullYear(), f(d.getHours()))}while(i--){if(a.a<a.c[i]) return $0.print('%s %s ago', Math.floor(a.a/a.c[1+i]), a.b[i])}},copy:function(a,b){var d=document,c=d.createElement('textarea');
c.value=a;
c.readOnly=1;
c.style.width='1px';
c.style.height='1px';
...
w.Art={cf:null, pg: 1,limit: 20,do: {},page: function(a){if(!(a.total>a.limit))return null;
...
return out+'</p>'},copy:function(a){a.style.fontWeight='700',$0.copy(a.dataset.abc)},
...
while(s < j){g+=$0.print(b, s, d[s++]);
}c.append(g).append(this.page(ob));
},

app:function(a,z){this.pg=z[2].substr(1)*1;
...
this.do[a[1]]=e;
this.dom(e);
}))},init: function(a,b,c){if('art'==b) return this.art(a,c);
...
return __c},home: function(a){var __a='<ul class="lobj">%s</ul>';
...
}},cfg: {bug:[], dm:[], sc:[], o:{id:0}, do:null},core:{a: function(){var a=this.b, b=a.a;
if(!b) return;
'art'==b && Art.app(a.b,a.c);
},b: {},c: {},e: {$0: /^(\w+)/,$1: function(a){if(/^.{0,1}$|^index\./.test(a)) return 'page/1';
return '6969~'},6969: {a:/^6969~/, b:0, c:0},page: {a:/^page\/(\d*)/, b:'home', c:2, d:'home'},emoji: {a:/^emoji\/(\w+)/, b:'%1', c:2, d:'art'},}},head:{tags:function(a,b,c){c=$0.print('/tag./%s', a.q.value.replace(/[\.\/]/g,'').trim().replace(/\s+/g,'+'));
if(200>c.length&&c.length>6){Z.onjs.a ? (new Z.on(2, null, [c, ''])) : w.open(c, '_self');
!!b && $0.popup(0)}a.q.blur && a.q.blur();
return !1},event:function(a){a.on('click', function(b){new Z.on(2, b, [this.pathname, this.search, this.hash])})}},debug:function(a,b){var c=this.cfg.bug, d=null;
if(!a){c.some(e=>{!!e && (e>0 ? clearTimeout(e) : e(69))});
return c.length=0, 1;
}if('function'==typeof a){d=c.push(setTimeout(function(){c[--d]=0;
a()}, b))}else{d=c.push(a.always(function(){c[--d]=0}).abort)}},on: function(a,b,c){if(2>a) c=[loc.pathname, loc.search, loc.hash];
return this.init(a,b,c)}};
Z.on.prototype={abc: null,main: null,exec: function(s,ss,z){var a=Z.core, b, c;
var d=a.e.$0.exec(s) || {};
b=a.e[d[1]];
...
}}return this.exec(a.e.$1(s))},init: function(a,b,c){a && Z.debug(null);
a>0 && w.stop && w.stop();
var d=Z.cfg, u=c.join(''), e=this.exec(c[0].replace(/^\/+/,''), c[1], c[2]);
...
Z.debug(()=>{this.$dom(Art.init(Z.cfg.do,e.b,e.a),a)}, 0==a?0:500)},get: function(a,b,c){Z.debug($.getJSON({url:c}).done(function(d){a.wait(3).csdl(b, d);
a.main.hide();
a.$dom(d, b.id, 0)}).fail(function(f,g){69==g || a.wait(0)}))},csdl: function(a,b,c){c=Z.cfg.dm;
if(1==b){return c.length=a+1, 0;
}if(!b) return c[a.id];
c[a.id]=b},sc__a: function(a,b,c){c=Z.cfg.sc;
...
c.empty().fadeIn(200).append(b)},$dom: function(a,b){var c=$('#apps__a'), d=$('#apps__b');
c.html(a.name);
